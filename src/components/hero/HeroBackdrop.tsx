...
function highlight(line: string, key: number) {
  ...
  const tokenRegex = /("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`|\b(?:const|let|var|function|return|if|else|for|while|import|from|export|default|new|class|extends|true|false|null|undefined|async|await|type|interface)\b|[{}()[\];,.<>=+\-*/!?:&|])/g;
  ...
  while ((m = tokenRegex.exec(codePart)) !== null) {
    ...
    const tok = m[0];
    if (tok.startsWith('"') || tok.startsWith("'") || tok.startsWith("`")) out.push({ t: tok, c: "str" });
  ...
  return (
    <span key={key}>
      {out.map((p, i) => {
        const cls =
          p.c === "kw" ? "text-[var(--code-syntax-1)]" :
          p.c === "str" ? "text-[var(--code-syntax-2)]" :
          p.c === "com" ? "text-[color:var(--code-comment)] italic" :
          p.c === "punc" ? "text-muted-foreground" :
          "text-foreground/80";
        return <span key={i} className={cls}>{p.t}</span>;
      })}
    </span>
  );
}
...
function SourcePanel() {
  ...
  return (
    <div
      ...
      style={{
        maskImage:
          "linear-gradient(to left, black 0%, black 94%, transparent 100%)",
        WebkitMaskImage:
          "linear-gradient(to left, black 0%, black 94%, transparent 100%)",
        opacity: 0.75,
      }}
            ...
            <div key={m.k} className="grid grid-cols-[2.4rem_4.5rem_1fr_3rem] items-center gap-3">
              ...
              <span
                className={`text-right text-[10px] uppercase tracking-[0.2em] ${
                  m.tag === "good" ? "text-accent" : m.tag === "fair" ? "text-foreground/70" : m.tag === "poor" ? "text-foreground/60" : "text-muted-foreground/60"
                }`}
              >
                {m.tag}
              </span>
          ...
          {[
            ...
            <div key={row.k} className="grid grid-cols-[6rem_1fr] gap-3">
              ...
              <dd className="flex items-center gap-2 tabular-nums text-foreground/80">
                {row.dot && <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-accent" />}
                {row.v}
              </dd>
            </div>
          ))}
    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      <div
        ...
        style={{
          ...
          WebkitMaskImage: "radial-gradient(ellipse 80% 65% at 50% 45%, black 35%, transparent 75%)",
        }}
      />
      <SourcePanel />
    </div>
